...
import { assignEmployee } from "@/app/actions/production"
import type { EmployeeOption } from "@/types/production"

interface ProductionAssignmentCardProps {
  jobId: string
  employees: EmployeeOption[]
  assignedEmployeeId: string | null
}

export function ProductionAssignmentCard({ jobId, employees, assignedEmployeeId }: ProductionAssignmentCardProps) {
  ...
  function handleAssign() {
    ...
    startTransition(async () => {
      const result = await assignEmployee(jobId, selected === "unassigned" ? "" : selected)
      if (result?.error) {
