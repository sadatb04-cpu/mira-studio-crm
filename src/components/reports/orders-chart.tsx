...
import type { OrdersByStatus } from "@/types/report"
...
export function OrdersChart({ data }: OrdersChartProps) {
  const chartData = data.map((row) => ({ label: ORDER_STATUS_LABELS[row.status], count: row.count }))
  const hasData = chartData.some((row) => row.count > 0)

  return (
    <SectionCard
      title="Orders by Status"
      description="Orders placed in the selected range."
      actions={
