...
import { deleteSellerInvoice, loadMoreSellerInvoices } from "@/app/actions/finance-sellers"
...
interface SellerInvoiceTableProps {
  sellerId: string
  invoices: FinanceSellerInvoiceListItem[]
  hasMore?: boolean
}

export function SellerInvoiceTable({ sellerId, invoices: initialInvoices, hasMore: initialHasMore = false }: SellerInvoiceTableProps) {
  const [invoices, setInvoices] = useState(initialInvoices)
  const [hasMore, setHasMore] = useState(initialHasMore)
  const [isLoadingMore, setIsLoadingMore] = useState(false)
  const [openingId, setOpeningId] = useState<string | null>(null)
  const [editingInvoice, setEditingInvoice] = useState<FinanceSellerInvoiceListItem | null>(null)
  const [deletingInvoice, setDeletingInvoice] = useState<FinanceSellerInvoiceListItem | null>(null)
  ...
  function handleLoadMore() {
    ...
    void (async () => {
      const page = await loadMoreSellerInvoices(sellerId, invoices.length)
  ...
  function handleOpenAttachment(invoice: FinanceSellerInvoiceListItem) {
  ...
  function handleDelete() {
    ...
    startTransition(async () => {
      const result = await deleteSellerInvoice(deletingInvoice.id, sellerId)
      if (result.error) {
  ...
  return (
    <SectionCard contentClassName="px-0">
        ...
        <TableBody>
          {invoices.map((invoice) => (
            <TableRow key={invoice.id}>
              ...
              <TableCell className="font-medium text-foreground">{invoice.product_name}</TableCell>
              <TableCell className="text-muted-foreground">{format(new Date(invoice.invoice_date), "MMM d, yyyy")}</TableCell>
              <TableCell className="text-right text-muted-foreground">{formatCurrency(invoice.manufacturing_price)}</TableCell>
              <TableCell className="text-right text-muted-foreground">{formatCurrency(invoice.selling_price)}</TableCell>
              <TableCell className={cn("text-right font-medium", invoice.profit >= 0 ? "text-success" : "text-destructive")}>
      ...
      <ConfirmDialog
        ...
        description={deleteError ?? `Are you sure you want to delete this ${deletingInvoice?.product_name ?? ""} invoice? This cannot be undone.`}
        confirmLabel="Delete"
        variant="destructive"
